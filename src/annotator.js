...
const _Annotator = this.Annotator;
...
class Annotator extends Delegator {
  static initClass() {
    ...
    this.prototype.options = // Configuration options
    ...
    this.prototype.viewer = null;
  
    this.prototype.selectedRanges = null;
  
    this.prototype.mouseIsDown = false;
  
    this.prototype.ignoreMouseup = false;
  
    this.prototype.viewerHideTimer = null;
  }
  ...
  _setupDynamicStyle() {
    ...
    const sel = `*${(['adder', 'outer', 'notice', 'filter'].map((x) => `:not(.annotator-${x})`)).join('')}`;
  ...
  highlightRange(normedRange, cssClass) {
    if (cssClass == null) { cssClass = 'annotator-hl'; }
    const white = /^\s*$/;

    const hl = $(`<span class='${cssClass}'></span>`);
  ...
  //
  // Returns an array of highlight Elements.
  highlightRanges(normedRanges, cssClass) {
    if (cssClass == null) { cssClass = 'annotator-hl'; }
    const highlights = [];
  ...
  addPlugin(name, options) {
    if (this.plugins[name]) {
      ...
      const klass = Annotator.Plugin[name];
...
Annotator.Plugin = class Plugin extends Delegator {
  constructor(element, options) {
    super(...arguments);
  }
